...
public class UserController {
    ...
    // Metodos creados para laa pruebas unitarias

    public boolean transactionValidation(double valueTransaction, double balance, int sId, int tId ){
        if(valueTransaction <= balance && sId != tId){
            return true;
        }
        return false;
    }

    public String printNewBalance( int idP,String nameP,double balanceP){
        String resul="Target User - ID: " + idP + ", Name: " + nameP + ", Balance: " + balanceP;
        return resul;
    }

    // Metodo sobre los cuales se utilizan los metodos que se prueban
    ...
    public boolean sendMoney(int sourceId, int targetId, double value, Context context) {
        ...
        System.out.println( printNewBalance(sourceUser.getId(),sourceUser.getName(),sourceUser.getBalance()));

        if (transactionValidation(value, sourceUser.getBalance(), sourceId, targetId)) {
